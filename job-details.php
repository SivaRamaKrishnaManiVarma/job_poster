...
$isExpired = $job['application_deadline'] && strtotime($job['application_deadline']) < strtotime(date('Y-m-d'));
    ...
    // Generate Job Posting Structured Data (Schema.org)
    $jobSchema = json_encode([
        "@context" => "https://schema.org/",
        "@type" => "JobPosting",
        "title" => $job['title'],
        "description" => strip_tags($job['description']) ?: $job['title'] . ' position at ' . $job['company'],
        "identifier" => [
            "@type" => "PropertyValue",
            "name" => $job['company'],
            "value" => $job['id']
        ...
        "employmentType" => match($job['type_name']) {
            'Full Time' => 'FULL_TIME',
            'Part Time' => 'PART_TIME',
            'Contract' => 'CONTRACTOR',
            'Internship' => 'INTERN',
        ...
        "hiringOrganization" => [
            "@type" => "Organization",
            "name" => $job['company'],
            "sameAs" => $job['official_website'] ?: null,
            "logo" => 'http://' . $_SERVER['HTTP_HOST'] . '/job_poster/assets/images/company-logo.png'
        ],
        "jobLocation" => [
            "@type" => "Place",
            "address" => [
                ...
                "addressRegion" => $job['state_name'] ?: null,
                "addressCountry" => "IN"
            ]
        ],
        "baseSalary" => $job['salary_max'] ? [
            "@type" => "MonetaryAmount",
            "currency" => "INR",
            "value" => [
        ...
        "experienceRequirements" => [
            ...
            "monthsOfExperience" => match($job['level_name']) {
                'Fresher' => 0,
                'Entry Level' => 12,
                'Mid Level' => 36,
                'Senior Level' => 60,
        ...
        "educationRequirements" => [
            ...
            "credentialCategory" => $job['qualification_name'] ?: "Not specified"
        ],
        "jobBenefits" => "Competitive salary and benefits package",
        "industry" => $job['category_name'] ?: null,
        "workHours" => $job['mode_name'] ?: null
    ], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);

    // SEO Meta Tags
    $pageTitle = $job['title'] . ' at ' . $job['company'] . ' - Job Portal';
    $metaDescription = substr(strip_tags($job['description']), 0, 155) . '... Apply now for this ' . $job['type_name'] . ' position in ' . $job['location'];
    ...
    $canonicalUrl = 'http://' . $_SERVER['HTTP_HOST'] . '/job_poster/job-details.php?slug=' . urlencode($job['slug']);
